...
class BlogImage(object):
    ...
    def everyday_view_num(cls):
        with SqlTool(db='blog_csdn') as cursor:
            cursor.execute('select sum(number) from read_number group by record_time order by record_time desc limit 30')
            view_num = cursor.fetchall()  # get the sum of view
            cursor.execute('select record_time from read_number group by record_time order by record_time desc limit 30')
